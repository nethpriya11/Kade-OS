...
interface KanbanColumnProps {
  ...
  onPrintReceipt: (order: Record<string, unknown>) => void;
}

const KanbanColumn = ({ title, status, color, icon: Icon, count, columnOrders, onUpdateStatus, onPrintReceipt }: KanbanColumnProps) => (
        ...
        {columnOrders.map(order => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <div className="flex flex-col items-end">
                ...
                <div className="flex gap-2">
                  <button onClick={() => onPrintReceipt(order)} className="p-1.5 bg-bg/50 rounded-lg hover:bg-primary hover:text-bg transition-colors" title="Print Receipt"><Printer size={14} /></button>
            ...
            <motion.button
              ...
              onClick={(e: React.MouseEvent) => { e.stopPropagation(); if (confirm('Cancel this order?')) onUpdateStatus(order.id, 'cancelled'); }}
...
const Orders = () => {
  ...
  const deductedOrders = React.useRef(new Set<string>());
  ...
  const fetchHistory = React.useCallback(async () => {
    ...
    if (data) setHistoryOrders(data as unknown as Order[]);
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    ...
    logOrderAction(user?.id, id, `status_${newStatus}`);
    ...
    <div className="h-full flex flex-col">
      ...
      {view === 'active' ? (
        <div className="flex-1 flex gap-6 overflow-x-auto pb-4">
          <KanbanColumn title="Pending" status="pending" color="secondary" icon={Clock} count={orders.filter(o => o.status === 'pending').length} columnOrders={orders.filter(o => o.status === 'pending')} onUpdateStatus={updateStatus} onPrintReceipt={printReceipt} />
          <KanbanColumn title="In Progress" status="in_progress" color="primary" icon={ChefHat} count={orders.filter(o => o.status === 'in_progress').length} columnOrders={orders.filter(o => o.status === 'in_progress')} onUpdateStatus={updateStatus} onPrintReceipt={printReceipt} />
          <KanbanColumn title="Ready for Pickup" status="ready" color="primary" icon={CheckCircle} count={orders.filter(o => o.status === 'ready').length} columnOrders={orders.filter(o => o.status === 'ready')} onUpdateStatus={updateStatus} onPrintReceipt={printReceipt} />
        </div>
      ) : (
