...
const Tables = () => {
    ...
    const fetchData = async () => {
        ...
        if (tablesData) setTables(tablesData as TableItem[]);
    ...
    const getTableOrder = (tableNum: number) => orders.find(o => o.table_number === tableNum);
                ...
                >
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p className="text-text-muted text-sm capitalize">{selectedTable.status} &middot; {selectedTable.capacity} seats</p>
                        </div>
                        <button onClick={() => setSelectedTable(null)} className="p-2 text-text-muted hover:text-text hover:bg-surface-hover rounded-xl transition-colors">&times;</button>
                    </div>
                    {getTableOrder(selectedTable.table_number) && (
                        <div className="mb-4 p-3 bg-surface rounded-xl border border-border">
                            ...
                            {getTableOrder(selectedTable.table_number)?.order_items?.map(item => (
                                <p key={item.id} className="text-sm text-text">{item.quantity}&times; {item.menu_items?.name}</p>
                            ))}
                        </div>
                    )}
