...
const Shifts = () => {
    ...
    const fetchData = async () => {
        ...
        setAllShifts((myShifts || []) as Shift[]);
        ...
        if (profile?.role === 'admin') {
            ...
            if (profilesData) {
                ...
                profilesData.forEach(p => {
                    map[p.id] = { id: p.id, full_name: p.full_name, username: p.username, role: p.role };
                });
                setProfiles(map);
            }
        }
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            const diff = new Date().getTime() - new Date(activeShift.clock_in).getTime();
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                    ...
                    <p className="text-2xl font-bold text-text">
                        {allShifts.filter(s => s.duration_minutes).length > 0
                            ? formatDuration(Math.round(allShifts.filter(s => s.duration_minutes).reduce((a, s) => a + (s.duration_minutes || 0), 0) / allShifts.filter(s => s.duration_minutes).length))
                            : '--'}
                    </p>
                </motion.div>
            </div>
                            ...
                            >
                                <div>
                                    ...
                                    <p className="text-sm text-text-muted mt-0.5">
                                        {formatTime(shift.clock_in)} &rarr; {formatTime(shift.clock_out) || <span className="text-green-400">Active</span>}
                                    </p>
                                </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    ...
                    <div className="glass rounded-3xl overflow-hidden">
                        <div className="p-6 border-b border-border flex items-center gap-3">
                            ...
                            <h2 className="font-bold text-text text-lg">Team Shifts &ndash; {new Date(filterDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</h2>
                        </div>
                        {teamShifts.length === 0 ? (
