...
const Inventory = () => {
    const { ingredients, logs: historyLogs } = useInventoryStore();

    const [editingId, setEditingId] = useState<string | null>(null);
    const [isSaving, setIsSaving] = useState(false);
    const [editValues, setEditValues] = useState<EditValues>({ stock: '', price: '', yield: '', threshold: '', category: '' });
    ...
    const [selectedForWastage, setSelectedForWastage] = useState<InventoryItem | null>(null);
    const [wastageData, setWastageData] = useState<WastageData>({ quantity: '', reason: '' });

    const [isRestockModalOpen, setIsRestockModalOpen] = useState(false);
    const [isRestocking, setIsRestocking] = useState(false);
    const [selectedForRestock, setSelectedForRestock] = useState<InventoryItem | null>(null);
    const [restockData, setRestockData] = useState<RestockData>({ quantity: '', price: '', totalCost: '', supplier_id: '' });
    ...
    }, [newIngredient.name, isCreating, allCategories]);

    const startEditing = (item: InventoryItem) => {
        setEditingId(item.id);
        setEditValues({
            stock: String(item.current_stock),
            price: String(item.purchase_price || 0),
            yield: String(item.yield_percent || 100),
            threshold: String(item.low_stock_threshold || 5),
    ...
    const handleSave = async (id: string) => {
        ...
        try {
            ...
            useInventoryStore.getState().updateIngredient(id, {
                current_stock: Number(editValues.stock),
                purchase_price: Number(editValues.price),
                yield_percent: Number(editValues.yield),
            ...
            toast.error(`Failed to update ingredient: ${(error as Error).message}`);
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const openWastageModal = (item: InventoryItem) => {
    ...
    const handleReportWastage = async () => {
        ...
        const yieldDecimal = (selectedForWastage.yield_percent || 100) / 100;
        const effectiveCost = yieldDecimal > 0 ? (selectedForWastage.purchase_price / yieldDecimal) : selectedForWastage.purchase_price;
        const costAtTime = effectiveCost * quantity;
        ...
        try {
            ...
            logInventoryAction(user?.id, selectedForWastage.id, 'wastage_deduct', {
                quantity, reason: wastageData.reason, cost_at_time: costAtTime,
            });
            if (logResult.data?.[0]?.id) {
                logWastageAction(user?.id, logResult.data[0].id, {
                    ingredient: selectedForWastage.name, quantity, reason: wastageData.reason,
                });
            }
            ...
            toast.error(`Failed to report wastage: ${(error as Error).message}`);
        } finally {
            setIsWastageSubmitting(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        if (confirm('Are you sure you want to delete this ingredient? This action cannot be undone.')) {
            try {
                ...
                logInventoryAction(useAuthStore.getState().user?.id, id, 'deleted');
                toast.success('Ingredient deleted');
            } catch (error) {
                console.error('Error deleting ingredient:', error);
                toast.error(`Failed to delete ingredient: ${(error as Error).message}`);
            }
        }
    };
    ...
    const openRestockModal = (item: InventoryItem) => {
        ...
        setRestockData({ quantity: '', price: String(item.purchase_price || ''), totalCost: '', supplier_id: '' });
        setIsRestockModalOpen(true);
    };

    const handleRestockChange = (field: string, value: string) => {
        const newData = { ...restockData, [field]: value };
    ...
    const handleRestock = async () => {
        ...
        const currentStock = parseFloat(String(selectedForRestock.current_stock || 0));
        ...
        try {
            ...
            logInventoryAction(useAuthStore.getState().user?.id, selectedForRestock.id, 'restocked', {
                quantity,
                total_cost: totalCost,
                new_price: newPrice
            });
            ...
            toast.error(`Failed to restock: ${(error as Error).message}`);
        } finally {
            setIsRestocking(false);
        }
    };
    ...
    const filteredIngredients = (ingredients as unknown as InventoryItem[]).filter(item => {
    ...
    const lowStockItems = items.filter(i => i.current_stock <= i.low_stock_threshold).length;
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setNewIngredient({ ...newIngredient, low_stock_threshold: Number(e.target.value) })}
                                        className="w-full bg-bg border border-border rounded-xl px-4 py-3 text-text focus:border-primary focus:outline-none"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setNewIngredient({ ...newIngredient, yield_percent: Number(e.target.value) })}
                                        className="w-full bg-bg border border-border rounded-xl px-4 py-3 text-text focus:border-primary focus:outline-none"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-red-400/80 text-xs mt-1">
                                        Estimated Loss: LKR {wastageData.quantity ? (() => { const yd = (selectedForWastage.yield_percent || 100) / 100; const ec = yd > 0 ? (selectedForWastage.purchase_price / yd) : selectedForWastage.purchase_price; return (ec * parseFloat(wastageData.quantity)).toFixed(2); })() : '0.00'}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-bg border border-border rounded-xl px-4 py-3 text-text focus:border-primary focus:outline-none"
                                        placeholder={String(selectedForRestock.purchase_price)}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-green-400/80 text-xs mt-1">
                                        New Total: {parseFloat(String(selectedForRestock.current_stock)) + (parseFloat(restockData.quantity) || 0)} {selectedForRestock.unit}
                                    </p>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-surface border border-border rounded-3xl p-6 w-full max-w-2xl shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
                        ...
                        <div className="overflow-y-auto flex-1 -mx-2 px-2">
                            {(historyLogs as unknown as RestockLog[]).length === 0 ? (
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody>
                                        {(historyLogs as unknown as RestockLog[]).map(log => (
