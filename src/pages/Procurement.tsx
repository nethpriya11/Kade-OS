...
const Procurement = () => {
    ...
    const [procurementItems, setProcurementItems] = useState<Record<string, ProcurementItem>>({});
    ...
    const fetchSupplierPrices = async () => {
        ...
        if (data) {
            ...
            (data as SupplierPrice[]).forEach(sp => {
    ...
    const fetchProcurementOrder = async () => {
        ...
        if (existingOrder) {
            setProcurementOrderId(existingOrder.id);
            const itemsMap: Record<string, ProcurementItem> = {};
            (existingOrder.procurement_order_items as Array<{
                ingredient_id: string; quantity_to_buy: number;
                estimated_cost: number; supplier_note: string; status?: string;
            }> || []).forEach((poi) => {
                itemsMap[poi.ingredient_id] = {
                    ingredient_id: poi.ingredient_id,
                    quantity_to_buy: poi.quantity_to_buy,
                    estimated_cost: poi.estimated_cost || 0,
                    supplier_note: poi.supplier_note || '',
                    status: existingOrder.status,
                };
            });
            setProcurementItems(itemsMap);
        }
    };
    ...
    const ensureProcurementOrder = async () => {
        if (procurementOrderId) return procurementOrderId;
        const userId = await getUserId();
        const { data, error } = await supabase
            .from('procurement_orders')
            .insert([{ created_by: userId, status: 'needed' }])
            .select()
            .single();
        if (error) {
        ...
        setProcurementOrderId(data.id);
        return data.id;
    };

    const saveItemToDb = async (ingredientId: string, item: ProcurementItem) => {
        const orderId = await ensureProcurementOrder();
        if (!orderId) return;

        const existing = procurementItems[ingredientId];
        if (existing) {
            await supabase
    ...
    const updateStatus = async (id: string, status: string) => {
        const toBuy = toBuyAmount(items.find(i => i.id === id)!);
        const item: ProcurementItem = {
            ingredient_id: id,
            quantity_to_buy: toBuy,
            estimated_cost: toBuy * (items.find(i => i.id === id)?.purchase_price || 0),
            supplier_note: notes[id] || '',
            status,
        };
        setProcurementItems(prev => ({ ...prev, [id]: item }));

        await saveItemToDb(id, item);
        ...
        const userId = await getUserId();
        logProcurementAction(userId, procurementOrderId || '', `status_${status}`, { ingredient_id: id });
    ...
    const saveNote = (id: string) => {
        ...
        if (procurementItems[id]) {
            supabase
                .from('procurement_order_items')
                .update({ supplier_note: tempNote })
                .eq('procurement_order_id', procurementOrderId)
                .eq('ingredient_id', id)
                .then(({ error }) => {
                    if (error) console.error('Failed to save note to DB:', error);
                });
        }
    };

    const toBuyAmount = (item: Ingredient) => Math.max(0, (item.low_stock_threshold || 20) - item.current_stock);
    ...
    const getStatus = (id: string) => procurementItems[id]?.status || 'needed';
