...
const PAYMENT_COLORS: Record<string, string> = { CASH: '#FFD700', CARD: '#4ECDC4', ONLINE: '#A78BFA' };
...
const getDateRange = (range: string, offset = 0) => {
    const now = new Date();
...
const Analytics = () => {
    ...
    const [metrics, setMetrics] = useState<Metrics>({ revenue: 0, orders: 0, avgOrderValue: 0, wastageCost: 0, netProfit: 0, totalExpenses: 0, taxCollected: 0 });
    const [prevMetrics, setPrevMetrics] = useState<PrevMetrics>({ revenue: 0, orders: 0 });
    ...
    const processAnalytics = useCallback((orders: Order[], wastage: WastageLog[], expenses: Expense[]) => {
        ...
        const hourMap: HourlyEntry[] = Array.from({ length: 24 }, (_, h) => ({ hour: `${String(h).padStart(2, '0')}:00`, orders: 0, revenue: 0 }));
    ...
    const fetchAnalytics = useCallback(async () => {
        ...
        if (compareMode) {
            ...
            const prevRevenue = (prevOrders || []).reduce((s: number, o: { total_amount: number }) => s + Number(o.total_amount || 0), 0);
            setPrevMetrics({ revenue: prevRevenue, orders: (prevOrders || []).length });
        }

        processAnalytics(orders, wastage, expenses);
        setOrdersList(orders);
    }, [timeRange, compareMode, processAnalytics]);
